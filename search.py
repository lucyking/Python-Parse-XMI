...
class XmiKeywords(object):
    ...
    def trans(self):
        ...
        filename = os.path.basename(self.fullname)
        dirname = os.path.dirname(self.fullname)
        file_object = codecs.open(self.fullname, 'rb')
    ...
    def traverse(self,path, depth=0):
        self.fullname = os.path.abspath(path)

        ret = re.search("\.xmi", self.fullname)
        if ret:
            print self.fullname,
            print os.path.basename(self.fullname),
            print os.path.dirname(self.fullname)
            self.trans()

        if (isdir(path)):
            for item in listdir(path):
                self.traverse(path + '/' + item, depth + 1)
